...
const SpeakersCard: React.FC<SpeakersCardProps> = ({
    ...
    const talkTitleStyles = {
        ...
        list: "text-lg font-bold mt-2 text-white",
    ...
    const titleStyles = {
        keynote: "",
        list: "text-lg ",
        selected: "text-base  font-medium",
        alternating: "text-sm"
    };
    ...
    if (variant !== 'alternating') {
        return (
            <div className={`${cardStyles[variant]} ${className}`}>
                <div className={` relative rounded-full overflow-hidden w-[${imageSizes[variant]}px] h-[${imageSizes[variant]}px] ${imageContainerStyles[variant]}`}>
